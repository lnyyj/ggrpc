...
import (
	...
	grpc_retry "github.com/lnyyj/ggrpc/middleware/retry"
	grpc_testing "github.com/lnyyj/ggrpc/middleware/testing"
	pb_testproto "github.com/lnyyj/ggrpc/middleware/testing/testproto"
...
func (s *RetrySuite) TestCallOptionsDontPanicWithoutInterceptor() {
	// Fix for https://github.com/lnyyj/grpcmid/issues/37
...
func TestChainedRetrySuite(t *testing.T) {
	...
	s := &ChainedRetrySuite{
		...
		InterceptorTestSuite: &grpc_testing.InterceptorTestSuite{
			TestService: service,
			ClientOpts: []grpc.DialOption{
				grpc.WithUnaryInterceptor(middleware.ChainUnaryClient(preRetryInterceptor.UnaryClientInterceptor, grpc_retry.UnaryClientInterceptor(), postRetryInterceptor.UnaryClientInterceptor)),
				grpc.WithStreamInterceptor(middleware.ChainStreamClient(preRetryInterceptor.StreamClientInterceptor, grpc_retry.StreamClientInterceptor(), postRetryInterceptor.StreamClientInterceptor)),
			},
		},
	}
	suite.Run(t, s)
